return {
  ...
  config = function()
    ...
    require("opencode").setup({
      ...
      config_file_path = vim.fn.stdpath("config") .. "/opencode-config.json", -- Custom config in nvim directory
      keymap = {
        global = {
          toggle = "<leader>oo", -- Open opencode. Close if opened
          open_input = "<leader>oi", -- Opens and focuses on input window on insert mode
          open_input_new_session = "<leader>oO", -- Opens and focuses on input window on insert mode. Creates a new session
          open_output = "<leader>oa", -- Opens and focuses on output window
        ...
        window = {
          ...
          prev_prompt_history = "<C-j>", -- Navigate to previous prompt in history
          next_prompt_history = "<C-k>", -- Navigate to next prompt in history
          switch_mode = "<C-b>", -- Switch between modes (build/plan)
      ...
      ui = {
        floating = true,
        window_width = 0.95, -- Full width
        input_height = 0.2, -- Input height as percentage of window height
        fullscreen = true, -- Start in fullscreen mode (default: false)
        layout = "center", -- Center layout works better for fullscreen
        floating_height = 0.95, -- Full height when floating
